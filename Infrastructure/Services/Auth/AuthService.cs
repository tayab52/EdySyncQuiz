...
namespace Infrastructure.Services.Auth
{
    public class AuthService : IAuthService
    {
        ...
        public ResponseVM SignUp(RegisterUserVM user)
        {
                ...
                && !string.IsNullOrWhiteSpace(user.Password))
            {
                ...
                try
                {
                    ...
                    var result = _appDBContext.Users.Add(userToSave);
                ...
                try
                {
                    ...
                    if (!firstUser.IsActive)
                    {
                        ...
                        response.Data = new
                        {
                            ...
                            otpSent = otpResponse.StatusCode == ResponseCode.Success
                        };
                        return response;
                    }
        ...
        {
            ResponseVM response = ResponseVM.Instance;
            var token = _appDBContext.RefreshTokens.FirstOrDefault(x => x.Token == refreshToken.RefreshToken);
            if (token == null)
            {
                response.StatusCode = ResponseCode.Unauthorized;
        ...
        public string GenerateJWT(Domain.Models.Entities.Users.User user)
        {
            ...
            _ = Convert.FromBase64String(_config["JWT:EncryptionKey"]!);
        ...
        public ResponseVM RefreshToken(TokenRequestVM refreshTokenRequest)
        {
            ...
            var user = _appDBContext.Users.Find(refreshToken.UserID);
            if (user == null)
            {
                response.StatusCode = ResponseCode.NotFound;
                response.ErrorMessage = "User not found.";
